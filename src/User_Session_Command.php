 ...
 * @package fin-cli
 */
class User_Session_Command extends FIN_CLI_Command {
	...
	public function destroy( $args, $assoc_args ) {
		...
		$manager = FIN_Session_Tokens::get_instance( $user->ID );

		if ( $token && $all ) {
			FIN_CLI::error( 'The --all flag cannot be specified along with a session token.' );
		}

		if ( $all ) {
			$manager->destroy_all();
			FIN_CLI::success( 'Destroyed all sessions.' );
			return;
		}
		...
		if ( ! $token ) {
			if ( empty( $sessions ) ) {
				FIN_CLI::success( 'No sessions to destroy.' );
			}
			$last  = end( $sessions );
			$token = $last['token'];
		}
		...
		if ( ! isset( $sessions[ $token ] ) ) {
			FIN_CLI::error( 'Session not found.' );
		}

		$this->destroy_session( $manager, $token );
		$remaining = count( $manager->get_all() );

		FIN_CLI::success( "Destroyed session. {$remaining} remaining." );
	}
	...
	protected function get_all_sessions( FIN_Session_Tokens $manager ) {
		// Make the private session data accessible to FIN-CLI
	...
	protected function destroy_session( FIN_Session_Tokens $manager, $token ) {
