...
class Post_Meta_Command extends CommandWithMeta {
	...
	public function clean_duplicates( $args, $assoc_args ) {
		...
		$metas = $findb->get_results(
			$findb->prepare(
				"SELECT * FROM {$findb->postmeta} WHERE meta_key=%s AND post_id=%d",
				$key,
				$post_id
			)
		);
		...
		if ( empty( $metas ) ) {
			FIN_CLI::error( sprintf( 'No meta values found for \'%s\'.', $key ) );
		}
		...
		if ( count( $dupe_metas ) ) {
			...
			foreach ( $dupe_metas as $meta_id ) {
				...
				FIN_CLI::log( sprintf( 'Deleted meta id %d.', $meta_id ) );
			}
			FIN_CLI::success( sprintf( 'Cleaned up duplicate \'%s\' meta values.', $key ) );
		} else {
			FIN_CLI::success(
				sprintf(
