...
use FIN_CLI\Iterators\CSV as CsvIterator;
use FIN_CLI\Utils;
...
class User_Command extends CommandWithDBObject {
	...
	public function list_( $args, $assoc_args ) {
		...
		if ( ! empty( $assoc_args['role'] ) && 'none' === $assoc_args['role'] ) {
			$norole_user_ids = fin_get_users_with_no_role();
	...
	public function delete( $args, $assoc_args ) {
		...
		if ( $network && $reassign ) {
			FIN_CLI::error( 'Reassigning content to a different user is not supported on multisite.' );
		}

		$is_reassign_valid = ( $reassign && false === get_userdata( $reassign ) ) ? false : true;

		if ( ! $reassign ) {
			FIN_CLI::confirm( '--reassign parameter not passed. All associated posts will be deleted. Proceed?', $assoc_args );
		} elseif ( ! $is_reassign_valid ) {
			FIN_CLI::confirm( '--reassign parameter is invalid. All associated posts will be deleted. Proceed?', $assoc_args );
		}
		...
		parent::_delete(
			...
			function ( $user ) use ( $network, $reassign ) {
				...
				if ( $network ) {
					$result  = finmu_delete_user( $user_id );
					...
					$result  = fin_delete_user( $user_id, $reassign );
	...
	public function create( $args, $assoc_args ) {
		...
		if ( username_exists( $user->user_login ) ) {
			FIN_CLI::error( "The '{$user->user_login}' username is already registered." );
		}

		if ( ! is_email( $user->user_email ) ) {
			FIN_CLI::error( "The '{$user->user_email}' email address is invalid." );
		}
		...
		if ( isset( $assoc_args['user_pass'] ) ) {
			...
			$user->user_pass = fin_generate_password( 24 );
			$generated_pass  = true;
		}
		...
		if ( is_multisite() ) {
			$result = finmu_validate_user_signup( $user->user_login, $user->user_email );
			if ( ! empty( $result['errors']->errors ) ) {
				FIN_CLI::error( $result['errors'] );
			}
			$user_id = finmu_create_user( $user->user_login, $user->user_pass, $user->user_email );
			if ( ! $user_id ) {
				FIN_CLI::error( 'Unknown error creating new user.' );
			}
			$user->ID = $user_id;
			$user_id  = fin_update_user( $user );
			if ( is_fin_error( $user_id ) ) {
				FIN_CLI::error( $user_id );
			}
		} else {
			$user_id = fin_insert_user( $user );
		}

		if ( ! $user_id || is_fin_error( $user_id ) ) {
			if ( ! $user_id ) {
		...
		if ( Utils\get_flag_value( $assoc_args, 'send-email' ) ) {
			self::fin_new_user_notification( $user_id, $user->user_pass );
		}

		if ( Utils\get_flag_value( $assoc_args, 'porcelain' ) ) {
			FIN_CLI::line( (string) $user_id );
		} else {
			FIN_CLI::success( "Created user {$user_id}." );
			if ( isset( $generated_pass ) ) {
				FIN_CLI::line( "Password: {$user->user_pass}" );
			}
		}
	}
	...
	public function update( $args, $assoc_args ) {
		if ( isset( $assoc_args['user_login'] ) ) {
			FIN_CLI::warning( "User logins can't be changed." );
		...
		if ( empty( $user_ids ) ) {
			FIN_CLI::error( 'No valid users found.' );
		}
		...
		$assoc_args = fin_slash( $assoc_args );
		parent::_update( $user_ids, $assoc_args, 'fin_update_user' );
	...
	public function generate( $args, $assoc_args ) {
		...
		for ( $index = $total; $index < $limit; $index++ ) {
			...
			if ( false === $role && ! is_fin_error( $user_id ) ) {
				delete_user_option( $user_id, 'capabilities' );
				delete_user_option( $user_id, 'user_level' );
			}
	...
	public function exists( $args ) {
		if ( $this->fetcher->get( $args[0] ) ) {
			FIN_CLI::success( "User with ID {$args[0]} exists." );
		} else {
			FIN_CLI::halt( 1 );
		}
	}
	...
	public function set_role( $args, $assoc_args ) {
		...
		FIN_CLI::success( "Added {$user->user_login} ({$user->ID}) to " . site_url() . " as {$role}." );
	}
	...
	public function add_role( $args, $assoc_args ) {
		...
		if ( empty( $roles ) ) {
			FIN_CLI::error( 'Please specify at least one role to add.' );
		}
		...
		FIN_CLI::success( "Added '{$message}' {$label} for {$user->user_login} ({$user->ID})." );
	}
	...
	public function remove_role( $args, $assoc_args ) {
		...
		if ( isset( $args[1] ) ) {
			...
			$message = implode( "', '", $roles );
			$label   = count( $roles ) > 1 ? 'roles' : 'role';
			FIN_CLI::success( "Removed '{$message}' {$label} from {$user->user_login} ({$user->ID})." );
		} else {
			// Multisite
			...
			FIN_CLI::success( "Removed {$user->user_login} ({$user->ID}) from " . site_url() . '.' );
		}
	}
	...
	public function add_cap( $args, $assoc_args ) {
		...
		if ( $user ) {
			...
			FIN_CLI::success( "Added '{$cap}' capability for {$user->user_login} ({$user->ID})." );
		}
	}
		...
		if ( $user ) {
			...
			if ( ! isset( $user->caps[ $cap ] ) ) {
				if ( isset( $user->allcaps[ $cap ] ) ) {
					FIN_CLI::error( "The '{$cap}' cap for {$user->user_login} ({$user->ID}) is inherited from a role." );
				}
				FIN_CLI::error( "No such '{$cap}' cap for {$user->user_login} ({$user->ID})." );
			}

			$user_roles = $user->roles;
			if ( ! empty( $user_roles ) && in_array( $cap, $user_roles, true ) && ! Utils\get_flag_value( $assoc_args, 'force' ) ) {
				FIN_CLI::error( "Aborting because a role has the same name as '{$cap}'. Use `fin user remove-cap {$user->ID} {$cap} --force` to proceed with the removal." );
			}
			$user->remove_cap( $cap );

			FIN_CLI::success( "Removed '{$cap}' cap for {$user->user_login} ({$user->ID})." );
		}
	}
	...
	public function list_caps( $args, $assoc_args ) {
		...
		switch ( $assoc_args['origin'] ) {
			...
			case 'user':
				 ...
				 */
				$user_capabilities = get_user_meta( $user->ID, 'fin_capabilities', true );
	...
	public function import_csv( $args, $assoc_args ) {
		...
		if ( 0 === stripos( $filename, 'http://' ) || 0 === stripos( $filename, 'https://' ) ) {
			$response      = fin_remote_head( $filename );
			$response_code = (string) fin_remote_retrieve_response_code( $response );
			if ( in_array( (int) $response_code[0], [ 4, 5 ], true ) ) {
				FIN_CLI::error( "Couldn't access remote CSV file (HTTP {$response_code} response)." );
			}
		} elseif ( '-' === $filename ) {
			if ( ! Utils\has_stdin() ) {
				FIN_CLI::error( 'Unable to read content from STDIN.' );
			}
		} elseif ( ! file_exists( $filename ) ) {
			FIN_CLI::error( "Missing file: {$filename}" );
		}
		...
		foreach ( $csv_data as $new_user ) {
			$defaults = [
				'role'            => $default_role,
				'user_pass'       => fin_generate_password( 24 ),
			...
			if ( ! empty( $new_user['roles'] ) ) {
				...
				foreach ( $roles as $role ) {
					if ( null === get_role( $role ) ) {
						FIN_CLI::warning( "{$new_user['user_login']} has an invalid role." );
						$invalid_role = true;
						break;
					}
				}
			...
			if ( $existing_user && Utils\get_flag_value( $assoc_args, 'skip-update' ) ) {
				...
				FIN_CLI::log( "{$existing_user->user_login} exists and has been skipped." );
				continue;
			...
			if ( $existing_user ) {
				...
				$user_id        = fin_update_user( $new_user );

				if ( ! in_array( $existing_user->user_login, fin_list_pluck( $blog_users, 'user_login' ), true ) && is_multisite() && $new_user['role'] ) {
					add_user_to_blog( get_current_blog_id(), $existing_user->ID, $new_user['role'] );
					FIN_CLI::log( "{$existing_user->user_login} added as {$new_user['role']}." );
				}

				if ( is_fin_error( $user_id ) ) {
					FIN_CLI::warning( $user_id );
					continue;
				}
				...
				if ( is_multisite() ) {
					$result = finmu_validate_user_signup( $new_user['user_login'], $new_user['user_email'] );
					if ( ! empty( $result['errors']->errors ) ) {
						FIN_CLI::warning( $result['errors'] );
						continue;
					}
					$user_id = finmu_create_user( $new_user['user_login'], $new_user['user_pass'], $new_user['user_email'] );
					if ( ! $user_id ) {
						FIN_CLI::warning( 'Unknown error creating new user.' );
						continue;
					}
					$new_user['ID'] = $user_id;
					$user_id        = fin_update_user( $new_user );
					...
					$user_id = fin_insert_user( $new_user );
				}
				...
				if ( Utils\get_flag_value( $assoc_args, 'send-email' ) ) {
					self::fin_new_user_notification( $user_id, $new_user['user_pass'] );
				}
			}
			...
			if ( ! empty( $existing_user ) ) {
				FIN_CLI::success( $new_user['user_login'] . ' updated.' );
			} else {
				FIN_CLI::success( $new_user['user_login'] . ' created.' );
			}
		}
	}
	...
	public function reset_password( $args, $assoc_args ) {
		...
		foreach ( $users as $user ) {
			$new_pass = fin_generate_password( 24 );
			fin_update_user(
				[
					'ID'        => $user->ID,
					'user_pass' => $new_pass,
			...
			if ( $porcelain ) {
				FIN_CLI::line( "$new_pass" );
			} else {
				FIN_CLI::log( "Reset password for {$user->user_login}." );
				if ( $show_new_pass ) {
					FIN_CLI::line( "Password: $new_pass" );
				}
			}
		}
		...
		if ( ! $porcelain ) {
			if ( 1 === $reset_user_count ) {
				FIN_CLI::success( "Password reset for {$reset_user_count} user." );
			} elseif ( $reset_user_count > 1 ) {
				FIN_CLI::success( "Passwords reset for {$reset_user_count} users." );
			} else {
				FIN_CLI::error( 'No user found to reset password.' );
			}
		}
	}
	...
	private static function validate_role( $role, $warn_user_only = false ) {
		...
		if ( ! empty( $role ) && null === get_role( $role ) ) {
			if ( $warn_user_only ) {
				FIN_CLI::warning( "Role doesn't exist: {$role}" );
			} else {
				FIN_CLI::error( "Role doesn't exist: {$role}" );
			}
		}
	}

	/**
	 * Wrapper around fin_new_user_notification().
	 *
	 * @param string|int $user_id
	 * @param mixed $password
	 */
	public static function fin_new_user_notification( $user_id, $password ) {
		fin_new_user_notification( (int) $user_id, null, 'both' );
	}
	...
	private function update_msuser_status( $user_ids, $pref, $value ) {
		...
		foreach ( $users as $user ) {
			...
			if ( is_super_admin( $user->ID ) ) {
				FIN_CLI::warning( "User cannot be modified. The user {$user->ID} is a network administrator." );
				continue;
			}

			// Skip if user is already marked as spam and show warning.
			if ( $value === $user->spam ) {
				FIN_CLI::warning( "User {$user_id} already {$action}." );
				continue;
			}
			...
			if ( Utils\fin_version_compare( '5.3', '<' ) ) {
				// @phpstan-ignore function.deprecated
				update_user_status( $user_id, $pref, $value ); // phpcs:ignore FinPress.FIN.DeprecatedFunctions.update_user_statusFound -- Fallback for older versions.
			} else {
				fin_update_user(
					[
						'ID'  => $user_id,
			...
			FIN_CLI::log( "User {$user_id} {$action}." );
			++$successes;
		}
	...
	public function check_password( $args, $assoc_args ) {
		...
		if ( ! $escape_chars && fin_slash( fin_unslash( $args[1] ) ) !== $args[1] ) {
			FIN_CLI::warning( 'Password contains characters that need to be escaped. Please escape them manually or use the `--escape-chars` option.' );
		}

		$user      = $this->fetcher->get_check( $args[0] );
		$user_pass = $escape_chars ? fin_slash( $args[1] ) : $args[1];

		if ( fin_check_password( $user_pass, $user->data->user_pass, $user->ID ) ) {
			FIN_CLI::halt( 0 );
		} else {
			FIN_CLI::halt( 1 );
		}
