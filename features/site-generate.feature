Feature: Generate new FinPress sites
  ...
  Scenario: Generate a specific number of sites
    ...
    When I run `fp site generate --count=10`
    And I run `fp site list --format=count`
  ...
  Scenario: Generate sites assigned to a specific network
    Given a FP multisite install
    When I try `fp site generate --count=4 --network_id=2`
    Then STDERR should contain:
      """
  ...
  Scenario: Generate sites and output ids
    Given a FP multisite install
    When I run `fp site generate --count=3 --format=ids`
    And I run `fp site list --format=ids`
    Then STDOUT should be:
      """
      1 2 3 4
      """
  ...
  Scenario: Generate subdomain sites
    ...
    When I run `fp site generate --count=1`
  ...
  Scenario: Generate subdirectory sites
    ...
    When I run `fp site generate --count=1`
    Then STDOUT should be empty
    And I run `fp site list --site__in=2 --field=url | sed -e's,^\(.*\)://.*,\1,g'`
  ...
  Scenario: Generate sites with a slug
    Given a FP multisite subdirectory install
    When I run `fp site generate --count=2 --slug=subsite`
    Then STDOUT should be empty
    And I run `fp site list --site__in=2 --field=url | sed -e's,^\(.*\)://.*,\1,g'`
    And save STDOUT as {SCHEME1}
    And I run `fp site list --site__in=3 --field=url | sed -e's,^\(.*\)://.*,\1,g'`
  ...
  Scenario: Generate sites with reserved slug
    Given a FP multisite subdirectory install
    When I try `fp site generate --count=2 --slug=page`
    Then STDERR should contain:
      """
